    ...
    return (
      <div className="min-h-screen bg-slate-50 grid place-items-center px-4">
        <div className="flex flex-col items-center gap-3 text-slate-400">
          ...
          <p className="text-sm">Loading signature request&hellip;</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4 py-16">
        <div className="max-w-lg w-full bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04),0_24px_48px_-24px_rgba(15,23,42,0.18)] border border-slate-200/70 rounded-2xl p-10 text-center">
          ...
          <p className="text-slate-500 mt-2.5">Your signature has been submitted successfully.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-50 py-10 px-4 sm:px-6">
      <div className="max-w-4xl mx-auto bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04),0_24px_48px_-24px_rgba(15,23,42,0.15)] border border-slate-200/70 rounded-2xl overflow-hidden">
        <header className="bg-gradient-to-br from-brand to-brand-dark text-white px-6 sm:px-10 py-9 text-center">
          ...
          <h2 className="text-base sm:text-lg font-medium text-white/90 mt-2">Signature Request</h2>
          <p className="text-sm text-white/70 mt-2">Company: {application.customerDetails.companyName}</p>
        </header>
        ...
        <div className="p-6 sm:p-10 space-y-8">
          ...
          {application.customerSignature?.signatureImage && (
            <section>
              <h3 className="text-base font-bold text-slate-900 mb-3 border-b border-slate-100 pb-2">Customer signature</h3>
              <img
                src={`${API_BASE.replace("/api", "")}${application.customerSignature.signatureImage}`}
                alt="Customer signature"
          ...
          {!verification ? (
            <div className="text-center border border-dashed border-slate-200 rounded-2xl p-8 bg-slate-50">
              ...
              <p className="text-slate-600 mb-5">
                Please verify your identity with Google using the email address this request was sent to
                (<span className="font-semibold text-slate-700">{data.signatureRequest.email}</span>) before signing.
              </p>
            ...
            <form onSubmit={handleSign} className="space-y-5 border-t border-slate-100 pt-7">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <TextInput label="Signatory Name" required value={signatoryName} onChange={(e) => setSignatoryName(e.target.value)} />
